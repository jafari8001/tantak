...
class autoTestCreator extends Command
{
    /**
     ...
     * @var string
     */
    protected $description = 'make basic test automatically 
    ...
    {
        return __DIR__ . '/../../../stubs/autoTest.stub';
    }
    ...
    public function getStubVariables()
    {
        ...
        $null_value_columns = $this->createCombinationalVariables($migration_fields);

        return [
            'MODULE_NAME'       => $this->option('module'),
            'MODEL_NAME'        => $this->getSingularClassName($this->option('model')),
            'LOWER_MODEL_NAME'  => strtolower($this->getSingularClassName($this->option('model'))),
    ...
    public function getSourceFilePath()
    {
        return base_path('tests/Feature') . '/' . 
            $this->getSingularClassName($this->option('model')) . 'Test.php';
    }
    ...
    protected function createCombinationalVariables($migration_fields)
    {
        ...
        foreach($migration_fields as $field){
            $null_value_columns .= "\t\t'$field' => '',".PHP_EOL;
        }

        $null_value_columns = substr($null_value_columns, 2,strlen($null_value_columns)-3);
